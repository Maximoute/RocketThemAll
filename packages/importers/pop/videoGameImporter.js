...
function determineRarity(rating, ratingsCount) {
    if (Math.random() < 0.01)
        return "Black Market";
    if (rating >= 4.5 && ratingsCount > 5000)
        return "Exotic";
    if (rating >= 4.2 || ratingsCount > 10000)
        return "Very Rare";
    if (rating >= 3.8 || ratingsCount > 3000)
        return "Rare";
    if (rating >= 3.4 || ratingsCount > 1000)
        return "Uncommon";
    return "Common";
}
const rarityXp = {
...
export async function importVideoGames(limit = 100, pages = 4) {
    const safeLimit = Math.max(1, Math.min(limit, MAX_IMPORT_LIMIT));
    const apiKey = process.env.RAWG_API_KEY;
    if (!apiKey)
        throw new Error("RAWG_API_KEY is not set. Add it to .env to import video games.");
    ...
    for (let p = 1; p <= pages && imported < safeLimit; p++) {
        const response = await withRetry(() => axios.get(`${RAWG_BASE}/games`, {
