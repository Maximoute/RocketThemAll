...
function determineRarity(score, members) {
    if (Math.random() < 0.01)
        return "Black Market";
    if (score >= 8.5 && members > 500000)
        return "Exotic";
    if (score >= 8.0 || members > 1000000)
        return "Very Rare";
    if (score >= 7.5 || members > 500000)
        return "Rare";
    if (score >= 7.0 || members > 100000)
        return "Uncommon";
    return "Common";
}
const rarityXp = {
...
async function importJikanType(type, limitPerType, pages) {
    ...
    for (let p = 1; p <= pages && imported < limitPerType; p++) {
        const response = await withRetry(() => axios.get(`${JIKAN_BASE}/top/${type}?page=${p}&limit=25`, { timeout: 10_000 }), `Jikan ${type} page ${p}`);
        const parsed = z.array(jikanItemSchema).safeParse(response.data.data || []);
        const items = parsed.success ? parsed.data : [];
        for (const item of items) {
            if (imported >= limitPerType)
                break;
...
export async function importAnimeAndManga(limit = 100, pages = 4) {
