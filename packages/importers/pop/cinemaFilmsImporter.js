...
const RARITY_ORDER_ASC = ["Common", "Uncommon", "Rare", "Very Rare", "Import", "Exotic", "Black Market", "Limited"];
const RARITY_ORDER_DESC = ["Limited", "Black Market", "Exotic", "Import", "Very Rare", "Rare", "Uncommon", "Common"];
const RARITY_TARGET_WEIGHTS = {
    Common: 0.50,
    Uncommon: 0.22,
    Rare: 0.12,
    "Very Rare": 0.07,
...
function clipAlias(alias, maxLen = 70) {
    if (alias.length <= maxLen)
        return alias;
    const clipped = alias.slice(0, maxLen).trim();
    return clipped.replace(/\s+\S*$/, "").trim();
}
function generateAcceptedNames(title) {
...
function normalizeTo100(value, bounds) {
...
function getXpByRarity(rarityName) {
    if (rarityName === "Limited")
        return 300;
    if (rarityName === "Black Market")
        return 250;
    if (rarityName === "Exotic")
        return 160;
    if (rarityName === "Import")
        return 110;
    if (rarityName === "Very Rare")
        return 70;
    if (rarityName === "Rare")
        return 40;
...
function getDropRateByRarity(rarityName) {
    if (rarityName === "Limited")
        return 0.01;
    if (rarityName === "Black Market")
        return 0.01;
...
function buildProportionalRarityCounts(total) {
    const result = Object.fromEntries(RARITY_ORDER_ASC.map((name) => [name, 0]));
    if (total <= 0)
        return result;
...
export function calculateMovieRarity(movie, bounds, acceptedNames) {
    const currentYear = new Date().getFullYear();
    ...
    const rarityScore = knownScore * 0.35
        + voteAverageN * 0.30
        + ageN * 0.20
        + classicScore * 0.25
    ...
    const finalRarityScore = clamp(rarityScore + acronymBoost + classicBoost + masterpieceBoost + cultBoost + recentPenalty + obscureOldPenalty, 0, 100);
    return {
...
async function fetchPopularMovies(maxPages) {
    const apiKey = process.env.TMDB_API_KEY;
    if (!apiKey)
        throw new Error("TMDB_API_KEY is not set");
    const all = [];
    for (let page = 1; page <= maxPages; page++) {
        const response = await withRetry(() => axios.get(`${TMDB_BASE}/movie/popular?page=${page}&language=fr-FR`, {
...
export async function importCinemaFilmsDeck(limit = 500, maxPages = 30) {
    ...
    const assignedRarities = new Map();
    let cursor = 0;
