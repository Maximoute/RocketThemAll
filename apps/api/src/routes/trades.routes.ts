...
router.patch("/:id", requireAuth, actionRateLimit, async (req, res) => {
  const params = validateParams(tradeIdSchema, req);
  ...
  if (action === "add") {
    ...
    return res.json(await tradeService.addItem(params.id, req.user!.id, cardId!, quantity ?? 1));
  }
  ...
  if (action === "remove") {
    ...
    return res.json(await tradeService.removeItem(params.id, req.user!.id, cardId!, quantity ?? 1));
  }

  if (action === "confirm") {
    return res.json(await tradeService.confirmTrade(params.id, req.user!.id));
  }

  return res.json(await tradeService.cancelTrade(params.id, req.user!.id));
});
